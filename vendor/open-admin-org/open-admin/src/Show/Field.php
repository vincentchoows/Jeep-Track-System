    ...
    */
    ...
    public function file($server = '', $download = true)
    {
        ...
        return $this->unescape()->as(function ($path) use ($server, $download, $field) {
            // Decode the JSON if it's a JSON-encoded string, otherwise just use the path
            $files = is_string($path) ? json_decode($path, true) : $path;
            $files = is_array($files) ? $files : [$path]; // Ensure $files is always an array
            $html = '<div class="card mailbox-arttachment clearfix" style="border: transparent;">';
            ...
            foreach ($files as $filePath) {
                ...
                if (url()->isValidUrl($filePath)) {
                    ...
                    if ($storage->exists($filePath)) {
                        ...
                        $size = ($storage->size($filePath) / 1000) . 'KB';
                    } else {
                        // File not found, display "Unavailable" message
                        $html .= <<<HTML
                    <div class="card-body" style="margin-bottom:0.5em;">
                        <div class="mailbox-attachment-name">
                            <i class="icon-paperclip"></i> {$name} (Unavailable)
                        </div>
                    </div>
                    HTML;
                ...
                $downloadAttr = $download ? "download='$name'" : '';
            ...
            <div class="card-body" style="margin-bottom:0.5em;">
                ...
                <span class="mailbox-attachment-size">
                    {$size}&nbsp;
                    <a href="{$url}" class="btn btn-light btn-xs float-end" target="_blank" $downloadAttr><i class="icon-download"></i></a>
                </span>
                <br>
            </div>
            HTML;
