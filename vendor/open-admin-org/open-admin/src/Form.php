...
use OpenAdmin\Admin\Actions\Interactor\Form as InteractorForm;
...
class Form implements Renderable
{
    ...
    {
        if ($this->model instanceof InteractorForm || $this->model instanceof Form ) {
    ...
    public function update($id, $data = null)
    {
        ...
        //NOTE: since all model will pass through here, check if its permit application model

        ////check if its permit application

        ////if yes, check submit value

        ////// if submit value = 3, update status = 3 

        ////// if submit value = 1 (approve), break (nothing happens);

        // $submitValue = $request->input('submit_value');
        // dd($submitValue);
